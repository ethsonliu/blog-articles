...
第一组比第二组多了 `if (tmp != m_pi)` 判断，理由很简单。两个管理同一份资源的 my_shred_ptr 相互赋值肯定是不可以再增加引用计数的
...
1. https://www.zhihu.com/question/56836057
2. https://blog.csdn.net/Solstice/article/details/8547547

## 思考

1. shared_ptr 支持多线程么？
2. 如果管理的资源 T，是线程安全，`shared_ptr<T>` 也是线程安全么？
3. shared_ptr 里的引用计数实现为什么要用线程安全操作？

## 参考

- [shared_ptr，weak_ptr 源码解析](https://blog.csdn.net/shangming111/article/details/70332095)
- [C++ 11 创建和使用共享 weak_ptr](https://www.cnblogs.com/DswCnblog/p/5628314.html)